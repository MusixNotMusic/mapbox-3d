 ...
 export const getDataFromBuffer = (buffer, byteType, byteOffset, byteLength, resType = 'number') => {
    ...
    switch (byteType) {
      ...
      case 'Int16':
        ...
        result = new DataView(buffer.slice(byteOffset, byteOffset + byteLength * 2), 0).getInt16(0, true);
        break;
      case 'Uint16':
        if (buffer.byteLength < byteOffset + byteLength * 2) throw new Error('sub-region exceeds array bounds.');
        result = new DataView(buffer.slice(byteOffset, byteOffset + byteLength * 2), 0).getUint16(0, true);
        break;
      case 'Int32':
        if (buffer.byteLength < byteOffset + byteLength * 4) throw new Error('sub-region exceeds array bounds.')
        result = new DataView(buffer.slice(byteOffset, byteOffset + byteLength * 4), 0).getInt32(0, true);
        break;
      case 'Uint32':
        if (buffer.byteLength < byteOffset + byteLength * 4) throw new Error('sub-region exceeds array bounds.');
        result = new DataView(buffer.slice(byteOffset, byteOffset + byteLength * 4), 0).getUint32(0, true);
        break;
      case 'Float32':
        if (buffer.byteLength < byteOffset + byteLength * 4) throw new Error('sub-region exceeds array bounds.');
        result = new DataView(buffer.slice(byteOffset, byteOffset + byteLength * 4), 0).getFloat32(0, true);
        break;
      case 'Float64':
        if (buffer.byteLength < byteOffset + byteLength * 8) throw new Error('sub-region exceeds array bounds.');
        result = new DataView(buffer.slice(byteOffset, byteOffset + byteLength * 8), 0, byteLength).getFloat64(0, true);
        break;
      case 'Uint64':
        if (buffer.byteLength < byteOffset + byteLength * 8) throw new Error('sub-region exceeds array bounds.');
        result = getUint64(new DataView(buffer.slice(byteOffset, byteOffset + byteLength * 8), 0), 0, true);
        break;
      default:
