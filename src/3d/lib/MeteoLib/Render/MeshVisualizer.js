...
import LOD from './LOD'
import TIFFParser from '../ThirdParty/tiff-js/tiff'
...
*@param {MeteoLib.Render.ArrowGeometry}[options.referenceAxisParameter]
*
...
*@property {MeteoLib.Render.Rotation}rotation 
...
*@example 
    ...
    MeshMaterial = MeteoLib.Render.MeshMaterial; 
    ...
    //示例1：Cesium.Geometry+MeteoLib.Render.MeshMaterial组合
    ...
    //示例2：Cesium.CSG+MeteoLib.Render.MeshMaterial组合，可以用Cesium.CSG做布尔运算并渲染运算结果
...
function MeshVisualizer(options) {
  ...
  this.referenceMesh = new ReferenceMesh({
    axisParameter: defaultValue(options.referenceAxisParameter, {
      length: 50000 * 2,
    }),
...
MeshVisualizer.prototype = {
  /**
   *@param {MeteoLib.Render.Mesh}mesh
   */
  ...
  getPickRay: function (windowPosition, result) {
    ...
    this._scene.camera.getPickRay(windowPosition, scratchRay) //这里射线的起始点是世界坐标，不能像Threejs那样直接拿来计算，需要转成局部坐标
    ...
    function onTextureImageLoaded(image, item) {
      ...
      if (defined(image.internalFormat)) {
        ...
        if (
          ...
          image.src.indexOf('data:image/png;') >= 0 ||
                ...
                Cesium.loadArrayBuffer(url)
                  .then(function (imageArrayBuffer) {
                    ...
                    if (that._textureCache[item.value]) {
                      that._textureCache[item.value].destroy &&
                        that._textureCache[item.value].destroy()
                    }
                    that._textureCache[item.value] = onTextureImageLoaded(
                      tiffCanvas,
                      item
                    )
                    callback.isLoading = false
    ...
    if (material.uniforms) {
      function setUniformCallbackFunc(name, item) {
        if (item !== undefined && item !== null) {
          ...
          if (
            ...
            (item.value instanceof Float32Array && item.value.length > 4)
          ) {
      ...
      for (var name in uniforms) {
        if (
          ...
          if (
            Cesium.isArray(uniforms[name].value) &&
            uniforms[name].value.length == 0
          ) {
            continue
          }
  ...
  getVertexShaderSource: function (geometry, material) {
    ...
    if (material.vertexShader) {
      ...
      var vs =
        getAttributeDefineBlok(material.vertexShader) +
        uniforms +
...
MeshVisualizer.prototype.update = function (frameState) {
  ...
  var sysWireframe =
    frameState.camera._scene._globe._surface.tileProvider._debug.wireframe
  if (this.debug) {
    sysWireframe = true
  }
  ...
  if (this._modelMatrixNeedsUpdate) {
    ...
    if (this._up && this._up.y) {
      this._actualModelMatrix = RendererUtils.yUp2Zup(
        this._actualModelMatrix,
        this._actualModelMatrix
      )
    }
  ...
  MeshVisualizer.traverse(
    ...
    function (mesh) {
      ...
      if (typeof mesh.update !== 'function') {
        ...
        mesh._drawCommand.uniformMap = that.getUniformMap(
          mesh.material,
          frameState
        )
        if (frameState.passes.pick) {
          ...
          mesh._drawCommand.renderState.depthTest.enabled =
 ...
 *
 */
MeshVisualizer.prototype._prepareFrameBufferTexture = function (
  frameState,
  frameBufferTexture,
  viewport
) {
    ...
    MeshVisualizer.traverse(
      ...
      function (mesh) {
        ...
        if (
          ...
          var notFullScreen =
            item._notFullScreen || Cesium.defined(item.texture)
          if (!notFullScreen) {
        ...
        // console.log('MeshVisualizer ==>', mesh._textureCommand.renderState)
...
MeshVisualizer.prototype.getPixels = function (
  ...
  if (item.drawCommands && item.drawCommands.length > 0) {
    if (
      ...
      item._computeTexture = new Texture({
        context: frameState.context,
    ...
    pixels = RendererUtils.renderToPixels(
      item.drawCommands,
      frameState,
      item._computeTexture,
      viewport,
...
/**
 *利用frameBufferTexture实现并行计算
 *@param {Cesium.FrameState}frameState
 *@param {MeteoLib.Render.FramebufferTexture}frameBufferTexture
 *@param {Object}viewport 视口设置
 *@param {Number}viewport.x 视口位置x坐标（屏幕坐标系，左上角为原点）
 *@param {Number}viewport.y 视口位置y坐标（屏幕坐标系，左上角为原点）
 *@param {Number}viewport.width 视口宽度
 *@param {Number}viewport.height 视口高度
 *@param {Cesium.PixelDatatype}viewport.pixelDatatype 指定输出数据中像素数据的数据类型
 *@param {Cesium.PixelFormat}viewport.pixelFormat 指定输出数据中像素数据的格式
 *@param {MeteoLib.Render.MeshVisualizer~computeDecodeFunc}[decodeFunc] 原始计算结果的解码回调函数,不传则直接返回原始的像素值rgba
 *@param {Array.<Number>}[result] 返回计算结果
 *@return {Array.<Number>}result 返回计算结果
 */
MeshVisualizer.prototype.compute = function (
  frameState,
  frameBufferTexture,
  viewport,
  decodeFunc,
  ...
  if (!result) {
    if (Cesium.isArray(decodeFunc)) {
      result = decodeFunc
      decodeFunc = undefined
    }
  }

  var pixels = this.getPixels(frameState, frameBufferTexture, viewport)
  if (typeof decodeFunc !== 'function') return pixels
  if (!result) result = new Float32Array(viewport.width * viewport.height)
  ...
  for (var row = 0; row < viewport.height; row++) {
    for (var col = 0; col < viewport.width; col++) {
      ;(rgba.r = pixels[pi++]),
        ...
        (rgba.b = pixels[pi++]),
        (rgba.a = pixels[pi++])
      result[index] = decodeFunc(rgba, row, col, index)
      index++
    }
  }
  return result
...
/**
 *@callback MeteoLib.Render.MeshVisualizer~computeDecodeFunc
 *@param {Object}rgba
 *@param {Number|Uint8}rgba.r
 *@param {Number|Uint8}rgba.g
 *@param {Number|Uint8}rgba.b
 *@param {Number|Uint8}rgba.a
 *@param {Number}row
 *@param {Number}col
 *@param {Number}index
...
MeshVisualizer.prototype.destroy = function () {
  ...
  MeshVisualizer.traverse(
    this,
    function (mesh) {
      if (mesh instanceof Mesh) mesh.destroy()
    },
    false
  )
 ...
 *
 *遍历节点
 *@param {MeteoLib.Render.MeshVisualizer|MeteoLib.Render.Mesh}root
 *@param {MeteoLib.Render.MeshVisualizer~TraverseCallback}traverseFunc 访问每个节点时回调该函数，进行相关操作。回调函数包含一个参数，traverseArgs，其中封装了一个属性cancelCurrent，可以通过改变此属性达到终止遍历当前节点的子节点
 *@param {Boolean}visibleOnly visibleOnly为true时仅遍历可见的节点，如果父级节点不可见则不再访问其子节点
 */
;(MeshVisualizer.traverse = function (
  node,
  traverseFunc,
  visibleOnly,
  scratchTraverseArgs
  ...
  /**
   *
   *@MeteoLib.Render.MeshVisualizer~TraverseCallback
   *@param {MeteoLib.Render.Mesh|MeteoLib.Render.LOD|MeteoLib.Render.MeshVisualizer|Object}node
...
MeshVisualizer._modifyShaderPosition = function (
  primitive,
  vertexShaderSource,
  scene3DOnly
) {
