...
GeometryUtils.mergeGeometries = function (geometries) {
  ...
  for (var i = 0; i < attrNames.length; i++) {
    ...
    newAttrs[attrName].values = new newAttrs[attrName].values.constructor(
      values
    )
  }
  var indices = []
 ...
 *@param {Number}
 */
GeometryUtils.setZ = function (geometry, z) {
  for (var i = 0; i < geometry.attributes.position.values.length; i += 3) {
    geometry.attributes.position.values[i + 2] = z
  }
}
 ...
 *@param {Number}
 */
GeometryUtils.setX = function (geometry, x) {
  for (var i = 0; i < geometry.attributes.position.values.length; i += 3) {
    geometry.attributes.position.values[i] = x
  }
}
 ...
 *@param {Number}
 */
GeometryUtils.setY = function (geometry, y) {
  for (var i = 0; i < geometry.attributes.position.values.length; i += 3) {
    geometry.attributes.position.values[i + 1] = y
  }
}
/**
 *
    ...
    if (geometry.attributes.hasOwnProperty(attrName)) {
      ...
      if (attr && attr.array.length > 0) {
        attributes[attrName] = new Cesium.GeometryAttribute({
          componentDatatype: GeometryUtils.getAttributeComponentType(
            attr.array
          ),
...
GeometryUtils.fromGeometry3js = function (geometry3js) {
  if (typeof THREE !== 'undefined') {
    if (
      geometry3js.attributes &&
      (geometry3js.index || geometry3js.groups.length)
    ) {
    } else {
    ...
    Cesium.GeometryPipeline.computeNormal(geometry)
    return geometry
  } else if (geometry3js.vertices && geometry3js.faces) {
    var positions = new Float32Array(geometry3js.vertices.length * 3)
    if (Cesium.isArray(geometry3js.vertices[0])) {
    } else {
...
;(GeometryUtils._fromCSG3js = function (csg_model) {
  ...
  (GeometryUtils._getGeometryVertice3js = function (
    geometry,
    vertice_position
  ) {
    var i
