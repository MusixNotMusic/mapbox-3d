 ...
 *@param {MeteoLib.Render.ArrowGeometry}[options.axisParameter]
...
function ReferenceMesh(options) {
  ...
  this._axisParameterY.reverse = Cesium.defaultValue(
    options.axisParameter.reverseY,
    false
  )
  ...
  var axisLine = ArrowGeometry.createGeometry(
    new ArrowGeometry(this._axisParameter)
  )
  var axisLineY = ArrowGeometry.createGeometry(
    new ArrowGeometry(this._axisParameterY)
  )
  ...
  if (this._axisParameterY.reverse)
    meshY.position.y = -this._axisParameter.length / 2
  else meshY.position.y = this._axisParameter.length / 2
...
Cesium.defineProperties(ReferenceMesh.prototype, {
  ...
  parent: {
    ...
    set: function (val) {
      if (
        val &&
        ((val._children && Cesium.isArray(val._children)) ||
          (val.children && Cesium.isArray(val.children)))
      ) {
